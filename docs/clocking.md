...
| `engine/wire_emit.rs` | Per-output PCR-anchored wire emission engine. Dedicated `std::thread` (Linux: `SCHED_FIFO` best-effort priority 50) pops TS datagrams off a `std::sync::mpsc::sync_channel(1024)` fed by the encoder task and emits each via `clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, target_ns)`. Target derivation: anchor on first PCR, re-anchor on every PCR (bounds drift to one inter-PCR period), interpolate non-PCR datagrams by declared bitrate (encoder targets + 5 % overhead) or EMA-inferred rate from inter-PCR pairs. Discontinuity > 500 ms or any backwards step resets the anchor. Optional RFC 2250 RTP wrap built in-thread so seq + SSRC track real wire egress. Channel overflow → `try_send` Full → `packets_dropped++` (no encoder back-pressure). **Status: module + tests landed; integration into output_udp / output_rtp deferred** — live testing showed the encoded path's bitstream PCRs are themselves non-uniform (offline `tsp pcrverify` on a captured TS file reproduces the jitter), which no wire-side pacing can correct. The eventual rollout depends on first fixing PCR placement in `ts_video_replace` / `av_sync_mux::pcr_for_emit`. Full doc: [`wire-pacing.md`](wire-pacing.md). |
