...
| C7 | Per-flow ingress rate limiting | **met** | `max_bitrate_mbps` on `RtpInputConfig` enables a token-bucket rate limiter. Excess packets are dropped before the broadcast channel. The token bucket uses integer arithmetic with a 10 ms burst allowance. Dropped packets are counted in `packets_filtered`. Additionally, `bandwidth_limit` on `FlowConfig` provides per-flow bandwidth monitoring with configurable actions: `alarm` (raise warning event + flag on dashboard) or `block` (gate the flow — drop all packets until bandwidth normalizes). Works across all input types (RTP, UDP, SRT, RTMP, RTSP, WebRTC). A configurable grace period (default 5s) avoids false positives from transient spikes. Block action uses a probe-and-check mechanism for automatic recovery. |
