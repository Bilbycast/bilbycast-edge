...
| warning | Media player output pacer falling behind schedule ({N} ms late) | The `ts` / `mp4` OS-thread pacer (`run_paced_emitter`) fell ≥ 250 ms behind its own computed wall-clock schedule — the signature of a source whose instantaneous byte rate (e.g. an oversized IDR or another bursty sample) outpaces the paced bitrate faster than the bounded hand-off queue can absorb. Latched — fires once per lagging episode. See issue #67 for the failure this exists to surface (an MP4 with a ~512 KB IDR silently produced no usable video with no alarm). | `{ error_code: "media_player_pacer_lagging", flow_id, input_id, pacer_lateness_ms, pacer_queue_depth, largest_video_sample_bytes }` |
