...
| `pes_splice_completed` | Info, **PES Switch Phase 4**. Audio-aligned splice committed — to-leg's first PUSI=1 PES with `pts ≥ last_a_pts + audio_frame_duration` was found inside the splice budget. Active leg has been flipped, PMT bumped (v+1 mod 32), DI=1 armed on next PCR, fresh PSI pushed. Receiver should see a glitchless cut. | `{ program_number, out_pid, to_input_id, first_b_pts }` | Operator-visible confirmation that the PES-aligned path actually fired. Without this event the operator can't tell whether the splice mode is having any effect. |
| `pes_splice_timeout` | Warning, **PES Switch Phase 4**. Audio-aligned splice budget expired without B producing an aligned PES — runtime fell back to today's `pmt_bump` path (immediate flip + PMT v+1 + DI=1). | `{ program_number, out_pid, to_input_id }` | Usually means B's stream rate is much slower than expected, or B isn't producing PUSI=1 in time. Bump `splice_budget_ms` if expected; otherwise inspect B's ingress for buffer/jitter issues. |
| `pes_splice_codec_param_mismatch` | Warning, **PES Switch Phase 4**. Audio-aligned splice arrived at the right PTS but B's AAC `AudioSpecificConfig` differs from A's (profile, sample rate, or channel configuration). A mid-PES splice would click; runtime refused the PES-aligned commit and fell back to today's `pmt_bump` path (immediate flip + PMT v+1 + DI=1) so receivers re-initialise the AAC decoder cleanly on the new params. Only fires for AAC ADTS (`stream_type 0x0F`) slots — AAC-LATM and other audio codecs commit on PTS alignment alone today. | `{ program_number, out_pid, to_input_id, a_aac_params: { profile, sample_rate_idx, sample_rate_hz, channel_config }, b_aac_params: { profile, sample_rate_idx, sample_rate_hz, channel_config } }` | Operator-actionable: receivers will still recover via the PMT bump, but the cut isn't glitch-free. Re-encode the legs to a common AudioSpecificConfig or switch the slot to `pmt_bump` deliberately to silence the event. |

**Source:** `src/engine/flow.rs` (`build_assembly_plan`, `non_ts_spts_error_code`, `resolve_essence_slots`), `src/engine/ts_assembler.rs` (`run_assembler` — splice commit / timeout), `src/engine/pes_splice.rs` (audio splice state machine), `src/config/validation.rs` (`validate_slot_source`). The edge manager-WS client also lifts these codes onto `command_ack.error_code` for `UpdateFlowAssembly` so the manager UI can highlight the offending field without needing the event stream.
