...
>
> **Where the shipped code differs from this sketch.** Five corrections, none of them changing the recommendation:
>
> - **The capability is three bits, not one.** The Design-sketch table's `"mxl"` capability row is wrong. `edge_capabilities()` pushes `mxl-video`, `mxl-audio` and `mxl-anc` together behind a single `probe_succeeded()` check (`src/manager/client.rs:1631-1634`), paralleling ST 2110-20/-30/-40 so per-essence build slicing stays open later. The bare string `"mxl"` does reach the manager, but on the **feature** list (`client.rs:1224`), not on `capabilities`.
> - **The release-channel question is settled.** No separate `*-linux-mxl` variant was minted: all three published artefacts request `mxl mxl-not-built` (`.github/workflows/nightly-release.yml:108`, `:114`, `:142`), and that happened without waiting for the quality gates below.
> - **The variant counts are a 2026-05-13 snapshot.** `InputConfig` now carries **23** variants and `OutputConfig` **20**. MXL added three to each and `Sdi` one to each, with `Mosaic` input-only — which does not account for the whole difference, so read 15 / 17 as stale rather than as a baseline to subtract from.
> - **The engine files landed one directory up.** `src/engine/mxl/` exists but holds `domain.rs` / `video.rs` / `audio.rs` / `ancillary.rs` / `grain_clock.rs`; the `input_mxl_*.rs` / `output_mxl_*.rs` dispatch shims sit at the engine root and delegate into `mxl_io.rs` / `mxl_video_io.rs`.
> - **The PTP requirement is asserted but not enforced.** The Design-sketch row demands `master_clock=wallclock` be refused on an MXL flow, and `MxlVideoInputConfig::clock_domain`'s rustdoc (`src/config/models.rs:7290-7291`) states it as done — but `src/config/validation.rs` has no such check anywhere. Auto-selection does put MXL inputs on `MasterClockKind::Ptp` (`src/engine/master_clock.rs:1560-1562`), so the default is right and only the default protects an operator; an explicit `master_clock.kind = "wallclock"` on an MXL flow is accepted today. That is a live code gap, not doc rot — implement the check or drop both claims.
>
> The FFI guidance in the Design sketch **was** followed and should not be read as stale: `bilbycast-mxl-rs/mxl-rs` declares `mxl = { path = "../vendor/mxl/rust/mxl" }` — a thin wrapper sitting on top of the vendored upstream bindings, exactly the shape the sketch permits.
