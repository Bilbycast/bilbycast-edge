...
| `codec` | string | No | `"h264_auto"` | Accepts the same names as an output's `video_encode.codec` — `h264_auto`, `hevc_auto`, `x264`, `x265`, `h264_qsv`, `h264_vaapi`, `h264_nvenc`, `h264_rkmpp`, … The canvas resolves it against the host's probed encoders, hardware first and CPU last. An unrecognised name is refused at save time, as is an explicit backend this build was not compiled with. Max 64 characters. Not honoured before v0.106.0 — the compositor opened the first backend `select_video_backend()` resolved and the configured name reached nothing (#129). |
