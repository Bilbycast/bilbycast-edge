...
impl IceAgent {
    ...
    fn stun_server_handle_message(&mut self, now: Instant, packet: &StunPacket) {
        ...
        // BILBYCAST PATCH (2026-04-09): paired with the parser-level patch
        // in `vendor/is/src/stun.rs` that drops the `STUN packet missing
        // priority` rejection. ffmpeg's WHIP muxer (and a few other
        // production publishers) sends Binding Requests without PRIORITY
        // even though RFC 5245 §7.1.2.1 makes it mandatory. Now that we
        // accept the packet at parse time, this `.expect()` would still
        // crash the agent task on the very same packet.
        //
        // Substitute a default ICE priority computed for a host candidate
        // (type_preference=126, local_preference=65535, component_id=1):
        //   prio = 126 * 2^24 + 65535 * 2^8 + (256 - 1)
        // This is the value RFC 5245 §4.1.2.1 prescribes for the standard
        // host candidate, which is what ffmpeg would have sent if it had
        // followed the spec. Pair selection therefore behaves as if
        // ffmpeg had emitted a normal ICE candidate.
        let prio = message
            .prio()
            .unwrap_or(126 * 16_777_216 + 65_535 * 256 + 255);
