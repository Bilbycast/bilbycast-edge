...
| `engine/` | `hardware_probe.rs` | **Resource-budget probe.** One-shot at startup: HW encoder / decoder presence via FFmpeg `avcodec_find_*_by_name` (NVENC, QSV, VideoToolbox, AMF), CPU brand + AVX class via `sysinfo` + `is_x86_feature_detected!`, static `(cores × avx_mult) → 720p30 x264 streams` heuristic. Optional NVML poll (`hardware-monitor-nvml` Cargo feature, Linux + Windows only) every 5 s for live NVENC / NVDEC % + active session count. Surfaced as `HealthPayload.resource_budget` with the `"resources"` capability string; manager UI keys the per-node Resources card and per-flow "Resource impact" tile off it. Cost-unit weights and the CPU heuristic are documented in [`docs/configuration-guide.md`](docs/configuration-guide.md#capacity--resource-budget). |
