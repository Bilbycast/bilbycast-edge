...
RIST support depends on one sibling crate set (no alternatives):
- **Pure Rust:** `rist-transport` + `rist-protocol` from `../bilbycast-rist/`

RIST is always compiled in — no feature flag, zero C deps. The `RistSocket::receiver` / `RistSocket::sender` public API is consumed directly by `engine::input_rist` / `engine::output_rist`. Wire-verified interop with librist 0.2.11 `ristsender` / `ristreceiver`.

## Feature Flags

The default build is LGPL-clean. Transcoder features are opt-in because they pull in GPL or vendor-specific libraries.
...
- `fdk-aac` — In-process AAC decode/encode via Fraunhofer FDK AAC — **default on**
- `video-thumbnail` — In-process video decode, JPEG thumbnails, and audio encoding (Opus/MP2/AC-3) via FFmpeg libavcodec/libswscale + libopus — **default on**
- `video-encoder-x264` — H.264 video transcoding via libx264 (forwarded to `video-engine`). **Default off.** Linking libx264 flips the whole FFmpeg build to GPL v2+, so binaries built with this feature inherit GPL terms. Requires `apt install libx264-dev` at build time.
- `video-encoder-x265` — HEVC video transcoding via libx265. **Default off.** Same GPL v2+ implication as x264. Requires `apt install libx265-dev`.
- `video-encoder-nvenc` — NVIDIA NVENC H.264/HEVC hardware encoders. **Default off.** LGPL-clean at the API layer, but requires an NVIDIA GPU + proprietary driver at runtime and `apt install nv-codec-headers` at build time.

Enable a transcoder backend by passing one or more of the `video-encoder-*` features, e.g. `cargo build --release --features video-encoder-x264`. Full reference: [`docs/transcoding.md`](docs/transcoding.md).
...
3. **Route-level RBAC**: public routes (`/health`, `/oauth/token`), read-only (any JWT), admin-only (mutations). Optional JWT auth on NMOS endpoints (`nmos_require_auth`, default `false`)
...
- Set `nmos_require_auth: true` in the `auth` config to require JWT Bearer auth on all NMOS endpoints (both `admin` and `monitor` roles have access)
