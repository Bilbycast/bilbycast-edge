...
All features are enabled by default. A plain `cargo build` includes everything.
...
- `video-thumbnail` — In-process video decode, JPEG thumbnails, and audio encoding (Opus/MP2/AC-3) via FFmpeg libavcodec/libswscale + libopus. Eliminates all ffmpeg subprocess dependencies — **default on**
