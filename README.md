...
1. **Prerequisites**: Install the [Rust toolchain](https://rustup.rs/) (stable, edition 2024). On Linux, also install the build-time system packages:
   ...
   ```
   `libasound2-dev` is the link-time dep of the on-by-default `display` Cargo feature (HDMI / DisplayPort + ALSA confidence-monitor playout); `drm-rs` is a pure-Rust ioctl crate (no libdrm-dev needed) and connector enumeration walks `/sys/class/drm` via std::fs (no libudev-dev needed). Building the `*-linux-full` variant additionally needs `libx265-dev libnuma-dev` (libx265 + libnuma are statically linked) plus a from-source static `libx264.a` (Debian/Ubuntu's `libx264-dev` ships no static archive), and `libvpl-dev` on x86_64 for QSV — full per-variant matrix in [docs/installation.md](docs/installation.md).
...
Each tagged release publishes three artefacts, all of them **full** builds (every video codec backend the edge knows about, bundled):

| Artefact                        | Target                          | Binary licence       |
|---------------------------------|---------------------------------|----------------------|
| `*-x86_64-linux-full`           | x86_64 Linux                    | AGPL-3.0-or-later combined work (bundles libx264 + libx265 under GPL-2.0-or-later; see `NOTICE.full` inside the tarball) |
| `*-aarch64-linux-full`          | ARM64 Linux                     | same as above |
| `*-aarch64-linux-rockchip`      | ARM64 Rockchip (RK3568 / RK3588) | same as above, plus RKMPP H.264/HEVC encode+decode and RGA DMA-buf transfer |

There is no separate default-only (`*-linux`, AGPL-without-encoders) artefact in the release matrix — the default feature set is still buildable from source (`cargo build --release`), it's just not published as a download. The runtime hardware probe auto-detects which compiled-in backend a host can actually open: NVENC/NVDEC on NVIDIA, QSV on Intel iGPU (x86_64 only), VAAPI on AMD (Mesa radeonsi) or Intel (iHD), RKMPP on the Rockchip artefact, and x264/x265 + libavcodec CPU decode everywhere else. Select `codec: "h264_nvenc"` / `"hevc_nvenc"` / `"h264_qsv"` / `"hevc_qsv"` (or `"h264_auto"` / `"hevc_auto"` to let the probe resolve) in your flow config on a matching host.

The full binaries link glibc 2.39+ dynamically (built on Ubuntu 24.04 / Debian 12+). libx264 + libx265 (and libnuma) are **statically linked**, so the binary needs **no codec runtime package** and is *not* tied to a distro's ABI-versioned `libx264.so`/`libx265.so` SONAME — which is exactly what broke earlier full builds on new Ubuntu releases (24.04 ships `libx264.so.164`, 26.04 ships `.165`, ABI-incompatible). The runtime apt packages it still needs are the stable-SONAME system libs: `libasound2` (on-by-default local-display output), plus `libva2 libva-drm2 libdrm2` for VAAPI and `libvpl2 intel-media-va-driver-non-free` for QSV on x86_64. NVENC needs only the proprietary NVIDIA driver. `install-edge.sh` installs these automatically. See [docs/installation.md](docs/installation.md) for download URLs and the install matrix.

**Commercial-licence customers**: a Softside Tech commercial licence covers the bilbycast source only — it cannot relicense libx264 / libx265, which remain GPL-2.0-or-later in the published full artefacts. Commercial deployments that need to avoid GPL copyleft should build from source with `video-encoder-nvenc` only (LGPL API layer; NVIDIA covers H.264/H.265 patents at the hardware layer) — the plain `cargo build --release` default feature set carries no software video encoders at all. See [LICENSE.commercial](LICENSE.commercial) for the full scope statement.
