...
impl TsDemuxer {
    ...
    pub fn demux(&mut self, ts_data: &[u8]) -> Vec<DemuxedFrame> {
        ...
        while offset + TS_PACKET_SIZE <= ts_data.len() {
            ...
            if pkt[0] == TS_SYNC_BYTE {
                if let Some(frame) = self.process_ts_packet(pkt) {
                    frames.push(frame);
                }
            }
    ...
    fn process_ts_packet(&mut self, pkt: &[u8]) -> Option<DemuxedFrame> {
        let pid = ts_pid(pkt);
    ...
    fn parse_pmt(&mut self, pkt: &[u8]) {
        ...
        while pos + 5 <= data_end {
            ...
            match stream_type {
                ...
                // AAC - we can detect it but can't transcode to Opus
                STREAM_TYPE_AAC_ADTS => {
                    tracing::debug!(
                        "TS demux: AAC audio PID 0x{:04X} detected (cannot transcode to Opus for WebRTC)",
                        es_pid,
                    );
                }
                _ => {}
    ...
    fn process_es_packet(&mut self, pkt: &[u8], pid: u16) -> Option<DemuxedFrame> {
        ...
        let completed_frame = if pusi {
            let assembler = self.pes_assemblers.get(&pid)?;
        ...
        let result = completed_frame
            .and_then(|(pes_data, stream_type)| self.parse_pes(&pes_data, pid, stream_type));

        // Now update the assembler
        let assembler = self.pes_assemblers.get_mut(&pid)?;
        if pusi {
    ...
    /// Parse a complete PES packet and extract the elementary stream frame.
    fn parse_pes(&mut self, pes: &[u8], pid: u16, stream_type: u8) -> Option<DemuxedFrame> {
